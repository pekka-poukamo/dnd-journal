...
// Sync Helper Functions - Simple functions for quick setup
const setLocalSyncServer = () => {
  try {
    localStorage.setItem('dnd-journal-sync-server', 'ws://localhost:1234');
    updateServerStatus();
    return { success: true };
  } catch (error) {
  ...
  if (statusElement) {
    try {
      ...
      statusElement.textContent = server 
        ? `Current: ${server}`
        : 'Current: Default public servers';
    } catch (e) {
      statusElement.textContent = 'Current: Error reading configuration';
    }
  }
};

// Setup quick sync buttons
const setupQuickSyncButtons = () => {
  const setLocalBtn = document.getElementById('set-local-server-btn');
  const clearServerBtn = document.getElementById('clear-server-btn');
  
  if (setLocalBtn) {
    setLocalBtn.addEventListener('click', () => {
      const result = setLocalSyncServer();
      if (result.success) {
        alert('✅ Local server set to ws://localhost:1234\n\nNext step: Run "npm run sync-server" in terminal');
      } else {
        alert('❌ Failed to set local server: ' + result.error);
      }
    });
  }
  ...
  if (clearServerBtn) {
    clearServerBtn.addEventListener('click', () => {
      const result = clearSyncServer();
      if (result.success) {
        alert('✅ Switched to public relay servers');
      } else {
        alert('❌ Failed to clear server: ' + result.error);
      }
    });
  }
...
const enhancedInit = () => {
  ...
  setupQuickSyncButtons(); // Add quick sync functionality
};
